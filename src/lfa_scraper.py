...
class LfaScraper(object):
    def __init__(self, file_location, scraper='Initial', cfg_location='yaml_cfgs/config.yaml'):
        ...
        with open(cfg_location) as stream:
            ...
            self.scraper_cfg = self.cfg['KeyDetailMatching'][scraper]
            self.matches = {k: re.search(vals['regex'], self.raw_txt, re.DOTALL) for k, vals in
                            self.scraper_cfg.items()}
